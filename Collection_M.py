...
base_dir = (
    r"C:\Users\Zaid\OneDrive\Desktop\lotlite\MagicBricks\Magicbricks_1_data_final"
)
        ...
        for file in files:
            ...
            try:
                with open(file_path, "r", encoding="utf-8") as f:
                    ...
                    avg_price = (
                        avg_price_tag.get_text(strip=True) if avg_price_tag else "N/A"
                    )
                    ...
                    summary_list = soup.find(
                        "div", class_="mb-srp__card__summary__list"
                    )
                    if summary_list:
                        status_tag = summary_list.find(
                            "div", {"data-summary": "status"}
                        )
                        possession_status = (
                            status_tag.find(
                    ...
                    if summary_list:
                        transaction_tag = summary_list.find(
                            "div", {"data-summary": "transaction"}
                        )
                        transaction = (
                            transaction_tag.find(
                                "div", class_="mb-srp__card__summary--value"
                            ).get_text(strip=True)
                            if transaction_tag
                            else "N/A"
